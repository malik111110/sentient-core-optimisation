...
# SurrealDB - Primary database with native async support
surrealdb = "^0.3.2"

# Event streaming (SurrealDB handles this natively via LIVE queries)
...
EVENT_BUS_BACKEND=surrealdb  # Native SurrealDB LIVE queries
...
class AsyncBaseAgent(ABC):
    ...
    async def _persist_checkpoint(self, checkpoint: Dict[str, Any]):
        """Persist checkpoint to SurrealDB storage."""
        try:
            from ..clients.surrealdb_client import get_surrealdb_client
            db = await get_surrealdb_client()
            if db:
                await db.create("agent_checkpoint", checkpoint)
                logger.debug(f"Persisted checkpoint {checkpoint['id']} to SurrealDB")
        except Exception as e:
    ...
    async def _load_checkpoint(self, checkpoint_id: str) -> Optional[Dict[str, Any]]:
        """Load checkpoint from SurrealDB storage."""
        try:
            from ..clients.surrealdb_client import get_surrealdb_client
            db = await get_surrealdb_client()
            if db:
                result = await db.query(
                    "SELECT * FROM agent_checkpoint WHERE id = $checkpoint_id",
