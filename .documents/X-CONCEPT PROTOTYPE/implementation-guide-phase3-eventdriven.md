...
class EventBus:
    """Async event bus with Redis backend for distributed messaging."""
    ...
    async def initialize(self):
        ...
        try:
            ...
            self._consumer_tasks = [
                ...
                asyncio.create_task(self._process_delayed_events()),
    ...
    async def publish(self, event: Event) -> bool:
        ...
        try:
            ...
            # Store in persistent storage for reliability
    ...
    async def subscribe(self, subscription: EventSubscription, handler: Optional[Callable] = None) -> bool:
        """Subscribe to events with optional local handler."""
        try:
            # Store subscription
            ...
            for event_type in subscription.event_types:
                ...
                # Redis subscription is handled by the consumer task
    ...
    async def _consume_events(self):
        """Consumer task for Redis pub/sub events."""
        if not self._redis:
            return
        ...
        try:
            ...
            logger.info(f"Event consumer started, listening to {len(channels)} channels")
            ...
            async for message in pubsub.listen():
                ...
                if message["type"] == "message":
                    try:
                        ...
                        await self._handle_distributed_event(event)
                    except Exception as e:
                        logger.error(f"Failed to process distributed event: {e}")
    ...
    async def _process_delayed_events(self):
        """Process events that need delayed delivery."""
        while self._running:
            try:
                await asyncio.sleep(60)  # Check every minute
                # Implementation for delayed/scheduled events
                # This would check for events with future delivery times
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Delayed event processing error: {e}")
    ...
    async def _persist_event(self, event: Event):
        ...
        try:
            if db:
                ...
                await db.create("event", event_data)
        except Exception as e:
    ...
    async def _load_subscriptions(self):
        ...
        try:
            if db:
                results = await db.query("SELECT * FROM event_subscription WHERE is_active = true")
    ...
    async def _remove_subscription(self, subscription_id: UUID):
        ...
        try:
            if db:
                await db.delete(f"event_subscription:{subscription_id}")
        except Exception as e:
            logger.error(f"Failed to remove subscription {subscription_id}: {e}")
        finally:
            if db:
